...
import type { genericRef } from '~~/types/other';
    ...
    const namePersonal: genericRef<string> = ref('')
...
<template>
    <section @click.stop="$emit('dropDialog')"
        ...
        <article @click.stop=""
            class="px-10 py-12 flex flex-col z-10 gap-10 bg-body-900 justify-start items-center rounded-xl border-3 border-white">
            ...
            <form class="flex flex-col gap-8 justify-center items-center">
                ...
                <button type="button" @click.stop.prevent="$emit('createPersonal', namePersonal)"
