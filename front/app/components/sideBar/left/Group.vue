...
watch(rooms, async () => {
    if (rooms.value?.length) {
        groupData.value.push(...rooms.value.filter((el) => el.type == "group"))
    }
})
    ...
    <article :class="{
        ...
        <section :class="{
            ...
            <article class="flex flex-col h-full gap-6 overflow-y-auto scrollbar-hide scroll-smooth w-full">
                ...
                ">
                    <section class="flex flex-row gap-10 jsutify-start items-center">
                        <img @click.stop="pickGroupAvatar(group.id)" :src="group.avatar || group.logo || '/img/userCall.png'"
                    ...
                    <section v-if="!activeGroup" class="flex items-center justify-center">
                        <svg @click.stop="copyInviteId(group.id)" :title="inviteFor(group.id)"
                    ...
                    <article>
                        ...
                        <section class="w-full flex border-b-3 border-white justify-center items-center p-2">
                            <button @click.stop="activeModelCreateRole = true"
                                class="border-1 text-white/50 hover:text-white px-5 py-2 text-[20px]  border-white w-3/4 ">{{
                                    settingUser.language == 'Englend' ? 'Create role' : 'Создать роль' }}</button>
                        </section>
                    </article>
                    <article>
