...
<template>
    <article @mouseenter="isActiveUser ? isHover = true : isHover = false" @mouseleave="isHover = false"
        ...
        <section
            :class='{ "w-fit flex flex-row-reverse gap-5 box-border": isActiveUser, "w-fit flex flex-row gap-5  box-border ": !isActiveUser }'>
            <section class=" relative flex justify-center w-20 h-20">
                <img class="w-15 h-15 rounded-full " :src='userMessage.srcImg' alt="lgogUser">
                <p class="absolute py-0.5 px-3 rounded-full bottom-[0%] bg-body-100 text-[18px] text-white/50">{{
                    userMessage.time }}</p>
            </section>
            <section
                ...
                <section v-if="isHover"
                    :class="{ 'flex flex-row gap-1 items-center absolute top-[-20px]': true, 'left-0': !isActiveUser, 'right-0': isActiveUser }">
                    <svg @click.stop="$emit('deleteMessage', messageData.id)" class="fill-red-500"
                    ...
                    <svg @click.stop="$emit('editMessage', messageData)" class="fill-white" width="25px"
                        height="25px" data-name="Layer 1" id="Layer_1" viewBox="0 0 32 32"
