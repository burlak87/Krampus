...
const joinLink = ref('')
const activeModalCreatePersonal = ref(false)
...
async function loadAll() {
    ...
    rooms.value = (await groupClass.requestGroup()).filter((room) => room.id.split('::').length == 1 && room.type == 'personal')
...
async function createPersonal(name: string) {
    ...
    activeModalCreatePersonal.value = false
    await loadAll()
}
...
<template>
    <ModalCreatePersonalChat v-if="activeModalCreatePersonal" @dropDialog="activeModalCreatePersonal = false"
        @createPersonal="createPersonal" />
    <article class="flex relative h-screen flex-col pt-10 px-2">
        ...
        <section class="flex flex-col pb-20 h-screen overflow-y-auto scrollbar-hide scroll-smooth gap-6">
            <section @click.stop="$emit('openChat', ['personal', el])"
                class="flex hover:bg-white/10 flex-row justify-between jutify-center items-center w-full cursor-pointer"
                v-for="el in rooms" :key="el.id">
                <section class="flex flex-row gap-10 items-center">
                    <img :src="el.avatar || el.logo || '/img/userCall.png'" class="w-10 h-10 rounded-full object-cover bg-white/10">
                    <p class="text-[18px] text-white font-bold">
                        {{ el.name }}</p>
                </section>
                <section class="flex items-center justify-center">
                    <svg @click.stop="copyInviteId(el.id)" :title="inviteFor(el.id)"
        ...
        <article
            ...
            @click.stop="activeModalCreatePersonal = true">
